...
public class PngImage //Forked from https://github.com/andburn/dds-reader
{
  private readonly IImage _image;

  public PngImage(string file) {
    var bytes = File.ReadAllBytes(file);
    _image = Dds.Create(bytes,new PfimConfig());
    Process();
  }

  public PngImage(Stream stream) {
    if (stream == null)
      throw new Exception("PngImage ctor: Stream is null");
  ...
  public PngImage(byte[] data) {
    if (data == null || data.Length <= 0)
      throw new Exception("PngImage ctor: no data");
